...
  };*/
  ...
  $.fn.listable = function() {
    return this.each(function() {
      $.data(this,"listable",new Listable(this));
    });
  };
  
  
  function Listable(elem, options)
  {
    this.init(elem, options);
  }
  
  
  Listable.prototype.init = function(elem, options) {
    this.element = elem;
    this.display('hide');
  };
  
  Listable.prototype.destroy = function() {
  };
  */
  ...
  Listable.prototype.updatePositions = function() {
  };
   */
  ...
  /**
   * Checked
  Listable.prototype.checked = function() {
  };
   */
  
  
  /**
   * Display roll over elements or hide them
  Listable.prototype.display = function(type) {
  };
   */
  
  
  /**
   * Edit mode for element
  Listable.prototype.edit = function() {
  };
   */
  ...
  /**
   * Delete this element
  Listable.prototype.delete = function() {
  };
   */
...
  $.fn.listable.checked = function(obj)
  {
    ...
    $.ajax({
      type: 'POST',
      url: $(obj).attr('rel-update-url')+'/'+$(obj).find('.check input').is(':checked')+'.js?objId='+$(obj).attr('id'),
      dataType: 'script',
      cache: false,
      success: function(response)
      {
...
  $.fn.listable.edit = function(obj)
  {
    ...
    var loading = $(obj).find('.loading');
    var name = $(obj).find('.label');
    var maintain = $(obj).find('.maintain');
    var checkbox = $(obj).find('.check');
    
    var url = $(obj).attr('rel-edit-url')+'?container=true&objId='+$(obj).attr('id');
